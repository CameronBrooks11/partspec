...
- `p.step_roundtrip(tol=)` — the part survives its own exchange format,
  ...
  calibrated relative tolerance (default 1e-9; healthy families measure
  ~1e-14, the executed degrader loses everything) and topology counts
  unchanged at any tolerance. The writer schema rides on the check
