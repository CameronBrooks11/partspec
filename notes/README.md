...
| ~~`file_issues.py`, `file_audit_issues.py`, `revise_audit_issues.py`~~ | **removed 2026-08-10** (v0.7.0 sweep). One-shot `gh` REST drivers. Their output — 53 issues, all since closed — lives on GitHub, which is a more durable and queryable store than the scripts. (53, counted by matching each issue title against the string literals in the scripts: 31 from `file_issues.py`, 22 from `file_audit_issues.py`, disjoint. This line first said 81, then 64; 64 is every issue created on 2026-08-06/07, and 11 of those were filed by hand one and two at a time — a second proxy substituted for the first, which is why the number is now derived from the scripts rather than from a date range.) `file_issues.py`'s own docstring says it "does not re-run safely", and `file_audit_issues.py` is only "idempotent-ish" (it refuses a duplicate title and nothing more). The pre-filing text is recoverable with `git show <sha>:notes/file_issues.py`; the revision history is already visible in the issue bodies, because `revise_audit_issues.py` appended auditable sections rather than rewriting. Nothing outside this directory ever referenced them. |
